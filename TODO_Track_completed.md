...
## Sim2Real deployment: @Asavari,@Chirag --> Done
...
- [x]: A few things to keep in mind before deploying:
     a. Make sure there are proper constraints on the values the policy can output -> Clip the values action values to be within a range. --> optional (?)
- [x]: _Compatibality check bw quadruped locomotion and unitree rl lab: check observation space and actor critic network structure._ --> Not needed now since we're training in unitree_rl_mjlab
...
- [x]: Is it possible to use the [quadrupeds_locomotion](https://github.com/Argo-Robot/quadrupeds_locomotion) checkpoint and deploy it in [untiree_rl_lab](https://github.com/unitreerobotics/unitree_rl_lab) --> @om and @chirag [05-02-2026] 
...
- [x]: What component from the robust deployment paper needs to be incorporated - https://leggedrobotics.github.io/rl-perceptiveloco/  --> @asavari
...
- [x]: This repo -> https://github.com/awesomericky/quadruped-robot-belief-encoder contains partial implementation of the above paper, analysis needed. @asavari
...
- [x]: Clone the legged_gym(Parallel Training) codebases as submodules.
    ...
    2. #TODO - Can we use the one that depends on IsaacLab or Mujoco instead because IsaacGym is no longer supported? Will switching break things mentioned in the paper? @mrigaank -> done Is the current implementation of the paper runn-able? How does the setup look like? - Fork the existing implementation and try so that we can track the changes you make.
